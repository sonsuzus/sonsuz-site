...
Python Programlamaya Giriş yazı dizimizde [Python fonksiyonlarının nasıl tanımlandığını](https://sonsuzus.github.ip/posts/python-programlamaya-giris-fonksiyonlar), [parametre](https://program.sonsuz.us/search.html?q=parametre) eşleştirmenin ve varsayılan parametrelerin [nasıl işlediğini gördük](https://program.sonsuz.us/posts/python-programlamaya-giris-fonksiyon-parametreleri), çeşitli örnekler ve alıştırmalar işledik. Dizinin bütün yazılarına erişmek için [*Python Programlamaya Giriş*](http://sonsuzus.github.io/category/program) kategorimize bakabilirsiniz.

Fonksiyonların her zaman `def` komutuyla tanımlanması gerekmez. Başka bir yol, `lambda` komutunu kullanarak isimsiz (“anonymous” – belli bir isme bağlı olmayan) [fonksiyonlar](https://program.sonsuz.us/search.html?q=fonksiyonlar) yaratmaktır. İsimsiz fonksiyonlar, `def` komutuyla üretilen fonksiyonlara göre daha kısıtlıdırlar. Buna karşılık, bir fonksiyon nesnesi gereken yerlerde bazı kolaylıklar sağlarlar.
