...
redirect_from:
  - /posts/cpp_cppio/
toc: true
image: /img/cpp_cppio-44.png
---

C++ [I/O](https://program.sonsuz.us/search.html?q=i/o) sistemi akışlar aracılığıyla çalışır. [Akış](https://program.sonsuz.us/search.html?q=ak%C4%B1%C5%9F), bilgi üreten veya kullanan mantıksal bir aygıt olarak tanımlanabilir. Bir akış, I/O sistemi tarafından fiziksel bir cihaza bağlanır. Bağlı oldukları gerçek fiziksel cihazlar farklı olsa bile tüm akışların çalışma şekli aynıdır. Tüm akışlar aynı şekilde çalıştığından, aynı I/O fonksiyonları hemen hemen her tür fiziksel aygıtta çalışabilir. Örneğin, bir dosyaya veri yazmak için kullandığımız bir fonksiyonu bir yazıcıya veya ekrana yazmak için de kullanabiliriz.

## Akış sınıfları

C++ I/O sistemi fonksiyonları ile [sınıf](https://program.sonsuz.us/search.html?q=s%C4%B1n%C4%B1f) bildirimleri `<iostream>` başlık dosyasında yer alır.
...
## @@MASK_40@@> işlemcilerine çoklu görev tanımlama
...
## Akışlara veri gönderen @@MASK_42@@ Herhangi bir operatör fonksiyonu bir sınıfın üyesi olduğunda, ilk parametresi işlemci fonsiyonuna yapılan çağrıyı oluşturan nesnedir. Ayrıca bu nesne, operatör fonksiyonunun üyesi olduğu sınıfın bir nesnesidir. Herhangi bir işlemci için yapılan çoklu görev tanımlama ile oluşturulan bir fonksiyon bir sınıfın üyesi ise, en soldaki parametre bu sınıfın bir nesnesi olmalıdır.
{: .prompt-tip}
...
![cpp_cppio-44](/img/cpp_cppio-44.svg)
