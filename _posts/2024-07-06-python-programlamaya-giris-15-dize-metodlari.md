...
Python programlamaya giriş yazı dizimizin bu bölümünde dizelerin kullanımını daha ayrıntılı inceleyeceğiz ve [dize](https://program.sonsuz.us/search.html?q=dize) veri tipine ait olan metodların bazılarına örneklerle göz atacağız. Dizinin bütün yazılarına erişmek için [*Python Programlamaya Giriş*](https://program.sonsuz.us/category/program) kategorimize bakabilirsiniz.
...
Dize sınıfı içinde tanımlanmış pek çok [metod](https://program.sonsuz.us/search.html?q=metod) mevcut. Bunların tam bir listesini görmek için `dir(str)` komutunu verebilirsiniz.
