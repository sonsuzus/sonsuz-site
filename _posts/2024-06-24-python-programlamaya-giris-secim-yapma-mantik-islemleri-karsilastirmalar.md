...
`if`‘den sonra `tahmin`‘in `hedef`‘e eşit olup olmadığını yoklayan bir mantık ifadesi var. Bunun değeri ya doğru (`True`) ya da yanlış (`False`) olacak. [Eşitlik](https://program.sonsuz.us/search.html?q=e%C5%9Fitlik) şartı için `==` (iki tane eşit işareti) kullanıldığına dikkat edin.
