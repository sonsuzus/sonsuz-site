...
Algoritmaların koda geçirilmesi ([implementasyon](https://program.sonsuz.us/search.html?q=implementasyon)) içinse iyi [kodlama](https://program.sonsuz.us/search.html?q=kodlama) bilgisi gerekir. Rekabetçi programlamada çözümler belirli test caseler kullanılarak puanlanır. Bu yüzden sadece algoritmayı düşünerek bulmak yetmez, aynı zamanda bunun koda doğru bir şekilde geçirilmesi önemlidir.
...
Bu kitaptaki örnek çözümler C++ ile yazılmış olup standart kütüphanedeki [algoritma](https://program.sonsuz.us/search.html?q=algoritma) ve veri yapıları sıklıkla kullanılmıştır.  Çözümler C++11 formatında yazılmıştır ki bu format şu anki çoğu yarışmada kullanılabilmektedir. 
